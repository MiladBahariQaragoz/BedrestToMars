...
\begin{enumerate}
  ...
  \item \textbf{At 32 campaigns, tabular machine learning adds nothing to a simple curve.} The four families, and a
        pretrained tabular neural network added post hoc, find the muscle structure---pooled $R^2$ doubles or
        triples---but do not convert it into lower out-of-campaign error. That is a statement about how much signal
        the published literature contains, not about the algorithms.
...
well~\citep{vanderploeg2014}, this is the expected outcome, and it was prepared for in advance. The post-hoc run of
TabPFN answers the obvious rejoinder---that the families were simply too data hungry, and a model pretrained for
small tables would do better. It does not: a network that has seen millions of synthetic datasets reaches the highest
pooled $R^2$ of any tier-2 model and still loses to the curve out of campaign. A better prior over tabular functions
cannot supply information the columns do not carry.
...
Linear regression, random forest, SVR, gradient boosting & As stated; linear regression is penalised (ridge);
  a pretrained neural network (TabPFN) was added post hoc \\
...
families and a pretrained tabular neural network add nothing to a simple duration curve, because the sample size is the number of campaigns, not the number of
