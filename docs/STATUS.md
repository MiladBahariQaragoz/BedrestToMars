...
| `framework/tests/` | 170 checks in 16 files, run by `make test` |
...
`results/forecast_comparison.csv`, `results/forecast_predictions.csv`. All regenerated by
...
history, and its ranges are **overconfident** in both arms. More than half the gain comes from
three heavily published campaigns, so recognising a study cannot be ruled out; the one
campaign no paper reports, `nasa_utmb_c3`, still shows a gain of 0.67 pp.
...
`make test` alone runs the 170 checks. `make forecast` rebuilds tier 3 from the answer cache;
...
| Tier 3 cannot rule out recognition of published studies | More than half its gain comes from three much-published campaigns. An ablation - the same states with the descriptions stripped, or with the numbers scrambled - would separate the two explanations |
